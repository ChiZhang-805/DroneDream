...
either optimization quality or simulator fidelity can support a
customer deployment or release decision.
...
117-file output across two builds. The reference manifest pins both
chains without copying raw evidence. Its claim audit resolves reported
counts, rates, scenario rows, contract versions, phase counts, and Runtime
boundaries to immutable Git bytes; the companion receipt binds source,
PDF, audits, and reproduction commands for independent review under the
...
admission. Concurrent claims retain a unique accepted physical attempt, and
...
The verification matrix separates receipted software checks from broader
product claims. Protected mutation and report-export contracts are
described in A.1 and A.2; the table reports only frozen validation
receipts and the report's recomputed layout contract. Every count remains
limited to its checked source boundary and does not measure controller
quality, physical-simulator fidelity, real-flight safety, or causal
optimization benefit. This compact map prevents release verification
from being mistaken for a physical or optimizer-performance conclusion.
Unsupported cells remain unavailable rather than being normalized into a
passing result or presented as verified engineering evidence at this boundary.
...
\midrule
\endhead
Subject-focused bridge & 59 & 59 & Outcome/report/evidence
paths\tabularnewline
...
For Free accounts, the renderer places a native vector seal on each page
after pagination is final. The seal uses the DroneDream violet-magenta
identity at low opacity, intersects the body
...
decision. The export contract defines tier resolution, watermark
cardinality, iteration trace, missing-evidence behavior, and stable byte
generation. Its authority ends at export correctness; it does not
qualify controller quality, simulator performance, or real-aircraft
behavior in any simulated or physical study reported here or in
downstream release claims.
...
and fails the build on drift. Router rows support eligibility claims only;
mock campaigns support bounded software and synthetic-response claims;
component ablations support observed protocol differences under their
frozen offline router. Backend and frontend receipts establish software
validation, not provider, PX4/Gazebo, or real-flight qualification.
Reviewers can reproduce each value while seeing which stronger conclusion
remains unauthorized under the declared evidence boundary and which future
experiment is required to authorize it with reproducible evidence.
