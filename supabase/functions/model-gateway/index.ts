...
export type ManagedProvider = "openai" | "deepseek" | "qwen" | "kimi";
...
interface ManagedModelDefinition {
  provider: ManagedProvider;
  display_name: string;
  model: string;
}

export const MANAGED_MODELS: readonly ManagedModelDefinition[] = [
  { provider: "openai", display_name: "GPT 4.1", model: "gpt-4.1" },
  { provider: "openai", display_name: "GPT 5.1", model: "gpt-5.1" },
  { provider: "openai", display_name: "GPT 5.4", model: "gpt-5.4" },
  { provider: "deepseek", display_name: "DeepSeek V4 Flash", model: "deepseek-v4-flash" },
  { provider: "deepseek", display_name: "DeepSeek V4 Pro", model: "deepseek-v4-pro" },
  { provider: "kimi", display_name: "Kimi K2.6", model: "kimi-k2.6" },
...
function managedProvider(value: unknown): ManagedProvider {
  const candidate = typeof value === "string" && value.trim()
    ...
    : Deno.env.get("PLATFORM_LLM_PROVIDER")?.trim().toLowerCase() || "openai";
  if (
    candidate !== "openai" && candidate !== "deepseek" && candidate !== "qwen"
      && candidate !== "kimi"
  ) {
    throw new GatewayError("INVALID_REQUEST", "provider is invalid.", 400);
  }
  return candidate;
}

function managedModelDefinition(
  provider: ManagedProvider,
  value: unknown,
): ManagedModelDefinition {
  const candidate = typeof value === "string" ? value.trim() : "";
  const definition = MANAGED_MODELS.find((entry) =>
    entry.provider === provider && entry.model === candidate
  );
  if (!definition) {
    throw new GatewayError("INVALID_REQUEST", "model is invalid.", 400);
  }
  return definition;
}

function providerPresentation(provider: ManagedProvider, model: string): {
  display_name: string;
  model: string;
} {
  const definition = managedModelDefinition(provider, model);
  return {
...
export function buildModelCatalog(rows: readonly ModelPolicyRow[]): JsonRecord {
  const models = rows.flatMap((row) => {
    if (
      (row.provider !== "openai" && row.provider !== "deepseek" &&
        row.provider !== "qwen" && row.provider !== "kimi") ||
      typeof row.enabled !== "boolean" ||
      typeof row.assistant_enabled !== "boolean" ||
      typeof row.job_enabled !== "boolean" ||
    ...
    return MANAGED_MODELS.filter((entry) => entry.provider === row.provider).map((entry) => ({
      ...entry,
      enabled: row.enabled,
  ...
  return {
    ...
    policy_version: models.reduce(
      (version, model) => Math.max(version, model.policy_version),
      1,
    ),
    models,
  };
...
async function handleModels(request: Request): Promise<Response> {
  ...
  const { data, error } = await adminClient()
    ...
    .select("provider,enabled,assistant_enabled,job_enabled,version")
    .order("provider");
  if (error || !Array.isArray(data)) {
    throw error ?? new GatewayError(
  ...
  return jsonResponse(request, 200, {
    data: buildModelCatalog(data as ModelPolicyRow[]),
  });
}
...
async function handleGrant(request: Request): Promise<Response> {
  ...
  const provider = managedProvider(body.provider);
  const model = managedModelDefinition(provider, body.model).model;
...
function providerConfiguration(provider: ManagedProvider, model: string) {
  const prefix = `PLATFORM_${provider.toUpperCase()}`;
  const globalProvider =
    Deno.env.get("PLATFORM_LLM_PROVIDER")?.trim().toLowerCase() ||
    "openai";
  const globalFallback = (name: string): string =>
    globalProvider === provider ? Deno.env.get(name)?.trim() || "" : "";
  ...
  const baseUrl = (Deno.env.get(`${prefix}_BASE_URL`)?.trim() ||
    globalFallback("PLATFORM_LLM_BASE_URL") ||
    defaultBaseUrl).replace(/\/+$/u, "");
  ...
  const apiKey = Deno.env.get(`${prefix}_API_KEY`)?.trim()
    ...
    ||
    globalFallback("PLATFORM_LLM_API_KEY");
  const selectedModel = managedModelDefinition(provider, model).model;
  if (!apiKey) {
    throw new GatewayError(
      "SERVICE_NOT_CONFIGURED",
      `The ${provider} managed model is missing required server configuration.`,
      503,
    );
  }
  return {
...
async function handleChatCompletions(request: Request): Promise<Response> {
  ...
  const provider = managedProvider(grantRecord.provider);
  const selectedModel = managedModelDefinition(provider, grantRecord.model).model;
  const config = providerConfiguration(provider, selectedModel);
  ...
  if (provider === "openai") {
    ...
    // Conversation state is owned by DroneDream's tenant-bounded database.
    // Never let a provider retain or implicitly extend a cross-request thread.
    providerBody.store = false;
  } else {
    providerBody.max_tokens = config.maxOutputTokens;
  }
  ...
  providerJson.model =
    Deno.env.get(`PLATFORM_${provider.toUpperCase()}_MODEL_ALIAS`)?.trim() ||
    (Deno.env.get("PLATFORM_LLM_PROVIDER")?.trim().toLowerCase() === provider
      ? Deno.env.get("PLATFORM_LLM_MODEL_ALIAS")?.trim()
      : "") ||
    providerPresentation(provider, selectedModel).display_name;
...
if (import.meta.main) {
  Deno.serve((request) => handleModelGatewayRequest(request));
}
