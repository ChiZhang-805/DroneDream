...
function user(overrides: Partial<SafeAdminUser> = {}): SafeAdminUser {
  return {
    ...
    licensed_editions: ["universal", "sim"],
...
function dependencies(
  ...
  return {
    ...
    deleteUser: (_actor, targetUserId, reason) =>
      Promise.resolve({ deleted_user_id: targetUserId, deleted: true, reason }),
...
Deno.test("user deletion is owner-only, bounded, and cannot target the owner", async () => {
  ...
  const forbidden = await handleAdminConsoleRequest(
    request(`/users/${target}/delete`, {
      method: "POST",
      body: JSON.stringify({ reason: "Verified account deletion request" }),
    }),
    dependencies(limited),
  );
  assert(forbidden.status === 403, "users.delete must be required");
  ...
  assert(short.status === 400, "short deletion reasons must be rejected");

  const self = "22222222-2222-4222-8222-222222222222";
  const selfDelete = await handleAdminConsoleRequest(
    request(`/users/${self}/delete`, {
      method: "POST",
      body: JSON.stringify({ reason: "Verified account deletion request" }),
    }),
    dependencies(),
  );
  assert(selfDelete.status === 409, "the owner must not delete itself");
  ...
  const ok = await handleAdminConsoleRequest(
    ...
    dependencies(undefined, {
      deleteUser: (actor, userId, reason) => {
        received.push({ actor, userId, reason });
        return Promise.resolve({ deleted_user_id: userId, deleted: true });
      },
    }),
  );
  assert(ok.status === 200, "valid user deletion should reach the atomic RPC");
  assert(received[0]?.reason === "Verified account deletion request", "reason must be trimmed");
});
...
Deno.test("user export is fixed-schema, private, quoted, and formula-safe", async () => {
  ...
  assert(
    csv.startsWith(
      "id,display_name,email,created_at,last_sign_in_at,plan,billing_scope,organization_name,subscription_status,licensed_editions,period_consumed_ai_credits,period_remaining_ai_credits,period_request_count,period_total_tokens\r\n",
    ),
