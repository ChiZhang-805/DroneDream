...
Deno.test("organization access can hide management from a regular member", async () => {
  ...
  assert(body.data.authorized === false, "member must not see management access");
});

Deno.test("organization snapshot is compact and includes the four-edition portfolio", async () => {
  const response = await handleOrganizationConsoleRequest(request(), dependencies());
  const body = await response.json();
...
Deno.test("owner can add an existing account with an exact bounded request", async () => {
  ...
  assert(lookup === "pilot@example.test", "email must be normalized by the route");
  assert(
...
Deno.test("member role changes and removals pass only the server-derived actor", async () => {
  ...
  assert(roleResponse.status === 200 && removeResponse.status === 200, "mutations should pass");
  assert(actions[0] === `role:${ACTOR_ID}:${TARGET_ID}:member`, "role actor mismatch");
  assert(actions[1] === `remove:${ACTOR_ID}:${TARGET_ID}`, "remove actor mismatch");
});

Deno.test("organization mutations reject unknown fields, owner role, and invalid ids", async () => {
  const cases = [
    request("/members", "POST", { email: "pilot@example.test", role: "member", owner: true }),
    request("/members", "POST", { email: "pilot@example.test", role: "owner" }),
    request("/members/not-a-uuid", "PATCH", { role: "member" }),
  ];
  for (const req of cases) {
    const response = await handleOrganizationConsoleRequest(req, dependencies());
    assert(response.status >= 400, "invalid organization mutation must fail closed");
  }
});
...
Deno.test("administrator-limit conflicts are safe and do not leak database errors", async () => {
  const response = await handleOrganizationConsoleRequest(
    ...
    dependencies({
      setMemberRole: () => Promise.reject(new OrganizationConsoleError(
  ...
  assert(!text.includes("postgres") && !text.includes("service_role"), "error leaked internals");
});
...
Deno.test("unexpected organization failures are sanitized", async () => {
  ...
  assert(!text.includes("database raw secret"), "raw error must not be returned");
});
