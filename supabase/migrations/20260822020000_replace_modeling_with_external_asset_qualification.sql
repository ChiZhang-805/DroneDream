-- Retire new in-app Vehicle Studio artifacts without deleting historical rows.
-- New work across all five products is a source-bound external asset import and
-- qualification plan. The completion RPC remains the server-authoritative
-- edition/artifact boundary, so update it in the same migration as the check.
...
alter table public.assistant_artifacts
  add constraint assistant_artifacts_artifact_kind_check check (
    artifact_kind in (
      'autonomy_mission_plan', 'external_asset_qualification_plan',
...
begin
  ...
  if not (
    ...
    or p_artifact_kind = 'external_asset_qualification_plan'
    or (selected_run.edition = 'universal' and p_artifact_kind in (
      'universal_simulation_experiment', 'universal_cross_edition_workflow'))
    ...
    or (selected_run.edition = 'autonomy' and p_artifact_kind = 'simulation_experiment')
  ) then
