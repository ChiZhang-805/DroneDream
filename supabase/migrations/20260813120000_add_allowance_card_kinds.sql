...
-- Keep one full-refill preview card for the product owner, then add three
-- fixed-credit cards. Existing redeemed cards and credentials are untouched.
with ranked as (
  select cards.card_id, row_number() over (order by cards.created_at, cards.card_id) as position
  from public.model_allowance_reset_cards cards
  join auth.users users on users.id = cards.user_id
  where lower(users.email) = 'cz91@illinois.edu'
    and cards.issued_reason = 'product-owner-preview'
    and cards.status = 'available'
)
update public.model_allowance_reset_cards cards
set status = 'revoked'
from ranked
where cards.card_id = ranked.card_id and ranked.position > 1;

insert into public.model_allowance_reset_cards (
  user_id, credits, card_kind, expires_at, issued_reason
)
select users.id, grants.credits, 'fixed_credit', now() + grants.valid_for, grants.reason
from auth.users users
cross join (values
  (1000::bigint, interval '6 months', 'product-owner-gift-1000'),
  (5000::bigint, interval '9 months', 'product-owner-gift-5000'),
  (10000::bigint, interval '1 year', 'product-owner-gift-10000')
) as grants(credits, valid_for, reason)
where lower(users.email) = 'cz91@illinois.edu'
  and not exists (
    select 1 from public.model_allowance_reset_cards existing
    where existing.user_id = users.id and existing.issued_reason = grants.reason
  );
...
begin
  select id into owner_id from auth.users where lower(email) = 'cz91@illinois.edu' limit 1;
  if owner_id is null then
    raise exception using errcode = 'P0001', message = 'PRODUCT_OWNER_ACCOUNT_NOT_FOUND';
  end if;
  select count(*) into available_full
  from public.model_allowance_reset_cards
  where user_id = owner_id and status = 'available' and card_kind = 'full_refill';
  select count(*) into available_gifts
  from public.model_allowance_reset_cards
  where user_id = owner_id and status = 'available' and card_kind = 'fixed_credit'
    and issued_reason in (
      'product-owner-gift-1000',
      'product-owner-gift-5000',
      'product-owner-gift-10000'
    );
  if available_full <> 1 or available_gifts <> 3 then
    raise exception using errcode = 'P0001', message = 'PRODUCT_OWNER_CARD_SET_INCOMPLETE';
  end if;
end;
$$;
