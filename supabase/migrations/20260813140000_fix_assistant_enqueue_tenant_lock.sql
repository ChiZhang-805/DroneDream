-- Serialize creation of an edition workspace conversation on its full tenant
-- boundary. The preceding hardening migration replaced the legacy uniqueness
-- key, so the enqueue function must not use the retired conflict inference.
