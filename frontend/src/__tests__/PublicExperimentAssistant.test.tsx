...
describe("public web assistant model boundary", () => {
  ...
  it("fixes the public console to the managed OpenAI model", () => {
    ...
    expect(modelSelector).toHaveValue("managed:openai");
    expect(modelSelector.querySelectorAll("option")).toHaveLength(1);
    expect(modelSelector).toHaveTextContent("GPT");
    expect(modelSelector).not.toHaveTextContent("DeepSeek");
