...
import { modelProviderDefinition } from "../features/settings/modelProviderCatalog";
...
export function ModelProviderLogo({ provider }: { provider: ModelProvider }) {
  if (provider === "together") {
    return (
      <svg
        className="model-provider-logo model-provider-logo-together"
        viewBox="0 0 24 24"
        aria-hidden="true"
      >
        <circle cx="8" cy="7.2" r="5.3" fill="#c3a3f7" />
        <circle cx="16" cy="7.2" r="5.3" fill="#e525c8" />
        <circle cx="12" cy="15.2" r="5.3" fill="#f45100" />
        <path d="M7.7 7.4 12 15m4.3-7.6L12 15m-4.3-7.6h8.6" fill="none" stroke="#fff" strokeWidth="1.2" strokeLinecap="round" />
      </svg>
    );
  }
  const path = PROVIDER_PATHS[provider];
  const definition = modelProviderDefinition(provider);
  const style = { "--model-provider-color": definition.color } as CSSProperties;
  if (!path) {
    return (
      <span
        className={`model-provider-custom-logo model-provider-custom-logo-${provider}`}
        style={style}
        aria-hidden="true"
      >
...
export function AssistantModelPicker({
  ...
  const selectedLabel = selectedDefault?.display_name
    ?? (selectedCustom?.displayName.trim() || selectedCustom?.model.trim())
    ?? chooseModelLabel;
    ...
    <div className="assistant-model-picker" ref={rootRef}>
      ...
      {open ? createPortal(
        <div
          ...
          style={menuStyle}
        >
          <div className="assistant-model-group-label">{defaultGroupLabel}</div>
          {defaultModels.map((model) => {
            ...
            return (
              <button
                ...
                title={available
                  ? undefined
                  : `${model.display_name} · ${temporarilyUnavailableLabel}`}
          ...
          {showCustomSection ? (
            <>
              <div className="assistant-model-group-label assistant-model-custom-heading">
                {customGroupLabel}
              </div>
              ...
              >
                ...
                <span><strong>{addCustomModelLabel}</strong></span>
              </button>
            </>
          ) : null}
        </div>,
