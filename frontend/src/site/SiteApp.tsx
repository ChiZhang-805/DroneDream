  ...
  "zh-CN": {
    ...
    capabilityDetails: [
      ...
      [
        ...
        ["精度优先优化器组合", "在六种互补引擎间分配预算，只让通过统一复验的真实增益进入最终决策排名。"],
      ],
      [
    ...
    manualChapters: [
      ...
      ["4 · 创建第一次实验", ["选择使用模式、机型、PX4 版本、Gazebo 模型与世界。", "依次完成五个向导步骤；当前步骤通过验证前，后续步骤始终锁定。", "选择参数及范围，设置场景、航迹、约束和预算，同时确认所有候选值、验证规则和运行边界都符合当前任务要求后再创建实验、启动调优并留存完整配置以及全部关键复核证据。"]],
