...
const compiledEdition = process.env.VITE_DRONEDREAM_EDITION === "lab" ? "lab" : null;
...
const cases = [
  ...
  .map((testCase) => compiledEdition ? { ...testCase, edition: compiledEdition } : testCase)
  ...
  for (const tab of await dialog.getByRole("tab").all()) {
    ...
    const measurement = await dialog.evaluate((element) => {
      ...
      return {
        ...
        consumerProfile: element.getAttribute("data-settings-consumer"),
        presentationOnly: element.getAttribute("data-presentation-only"),
    ...
    assert.equal(measurement.consumerProfile, compiledEdition === "lab" ? "lab" : "universal");
    assert.equal(measurement.presentationOnly, "true");
