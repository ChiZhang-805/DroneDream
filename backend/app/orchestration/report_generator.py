...
def _ensure_immutable_file_artifact(
    ...
    *,
    ...
    if existing is not None:
        ...
        stored_content = storage.read_bytes(existing.storage_path)
        receipt = require_artifact_integrity(existing, content=stored_content)
        if receipt is None and stored_content != content:
            raise ArtifactIntegrityError(
    ...
    db.add(artifact)
    bind_artifact_integrity(db, artifact=artifact, content=stored_content)
