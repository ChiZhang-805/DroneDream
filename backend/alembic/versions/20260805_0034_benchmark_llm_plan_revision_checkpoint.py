"""persist bounded plan/revision checkpoints without provider replay
...
_TABLE = "benchmark_llm_plan_revision_checkpoints"
_UPDATE_TRIGGER = "trg_benchmark_plan_checkpoint_no_update"
_DELETE_TRIGGER = "trg_benchmark_plan_checkpoint_no_delete"
_POSTGRES_FUNCTION = "dronedream_reject_benchmark_plan_checkpoint_mutation"
_POSTGRES_TRIGGER = "trg_benchmark_plan_checkpoint_immutable"
...
def upgrade() -> None:
    op.create_table(
        ...
        sa.Column("turn_role", sa.String(length=16), nullable=False),
    ...
    op.create_index("ix_benchmark_llm_plan_revision_checkpoints_job_id", _TABLE, ["job_id"])
    op.create_index(
        "ix_benchmark_llm_plan_revision_checkpoints_run_binding_id",
        _TABLE,
        ["run_binding_id"],
    )
    op.create_index(
        "ix_benchmark_llm_plan_revision_checkpoints_cognitive_turn_receipt_id",
        _TABLE,
        ["cognitive_turn_receipt_id"],
        unique=True,
    )
    ...
    if dialect == "sqlite":
        ...
        raise RuntimeError("Benchmark plan checkpoint migration supports SQLite/PostgreSQL only")
...
def _install_sqlite_guards() -> None:
    op.execute(
        ...
        BEGIN
            SELECT RAISE(ABORT, 'benchmark plan checkpoints are append-only');
        END
        """
    )
    op.execute(
    ...
    op.execute(
        ...
        BEGIN
            IF TG_OP = 'DELETE' AND EXISTS (
                ...
                RETURN OLD;
            END IF;
            RAISE EXCEPTION 'benchmark plan checkpoints are append-only';
        END;
        $$
        """
    )
...
def downgrade() -> None:
    ...
    op.drop_index("ix_benchmark_llm_plan_revision_checkpoints_run_binding_id", table_name=_TABLE)
    op.drop_index("ix_benchmark_llm_plan_revision_checkpoints_job_id", table_name=_TABLE)
