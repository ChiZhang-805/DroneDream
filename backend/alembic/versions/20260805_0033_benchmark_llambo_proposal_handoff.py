"""persist UAV-constrained LLAMBO proposals without provider replay
...
_TABLE = "benchmark_llambo_proposal_handoffs"
_UPDATE_TRIGGER = "trg_benchmark_llambo_handoff_no_update"
_DELETE_TRIGGER = "trg_benchmark_llambo_handoff_no_delete"
_POSTGRES_FUNCTION = "dronedream_reject_benchmark_llambo_handoff_mutation"
_POSTGRES_TRIGGER = "trg_benchmark_llambo_handoff_immutable"
...
def upgrade() -> None:
    ...
    op.create_index("ix_benchmark_llambo_proposal_handoffs_job_id", _TABLE, ["job_id"])
    op.create_index(
        "ix_benchmark_llambo_proposal_handoffs_run_binding_id",
        _TABLE,
        ["run_binding_id"],
    )
    op.create_index(
        "ix_benchmark_llambo_proposal_handoffs_cognitive_turn_receipt_id",
        _TABLE,
        ["cognitive_turn_receipt_id"],
        unique=True,
    )
    ...
    if dialect == "sqlite":
        ...
        raise RuntimeError("Benchmark LLAMBO handoff migration supports SQLite/PostgreSQL only")
...
def _install_sqlite_guards() -> None:
    op.execute(
        ...
        BEGIN
            SELECT RAISE(ABORT, 'benchmark LLAMBO proposal handoffs are append-only');
        END
        """
    )
    op.execute(
    ...
    op.execute(
        ...
        BEGIN
            IF TG_OP = 'DELETE' AND EXISTS (
                ...
                RETURN OLD;
            END IF;
            RAISE EXCEPTION 'benchmark LLAMBO proposal handoffs are append-only';
        END;
        $$
        """
    )
...
def downgrade() -> None:
    ...
    op.drop_index("ix_benchmark_llambo_proposal_handoffs_run_binding_id", table_name=_TABLE)
    op.drop_index("ix_benchmark_llambo_proposal_handoffs_job_id", table_name=_TABLE)
