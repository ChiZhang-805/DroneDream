<p align="center">
  <img src="docs/assets/drone-dream-icon.png" alt="DroneDream icon" width="240" />
</p>
...
- 🚁 **真实仿真**：支持 `real_cli + px4_gazebo_runner.py` 接入 PX4/Gazebo。
- 🧠 **自动调参**：支持 heuristic 和 GPT 参数提议。
- 🖥️ **可视化**：支持 noVNC/Gazebo GUI、Trajectory replay 和 artifacts。
- 🛣️ **赛道 marker**：支持 Gazebo marker service fallback，兼容 `/world/<world>/marker` 与 `/marker`。
- 📄 **报告**：支持后端生成 PDF job report。
...
DroneDream/
  ...
  backend/      FastAPI + SQLAlchemy API 与报告/artifact 服务
  worker/       数据库轮询 worker，执行 baseline / heuristic / GPT trial
  scripts/      PX4/Gazebo runner、launcher、marker、dev helper scripts
  docs/         复现流程与工程文档
  .env.example  环境变量模板
```
...
所有公开 API 位于 `/api/v1`，并使用统一响应 envelope：
