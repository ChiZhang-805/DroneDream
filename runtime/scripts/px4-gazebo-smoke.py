...
async def _round_trip() -> dict[str, float | str]:
    ...
    written = min(
        2.0, max(0.0, original + 0.05 if original <= 1.95 else original - 0.05)
    )
    ...
    try:
        ...
        await asyncio.wait_for(
            drone.param.set_param_float(parameter, written), timeout=30
        )
        read_back = float(
            await asyncio.wait_for(drone.param.get_param_float(parameter), timeout=30)
        )
    ...
    if abs(written - read_back) > 1e-4:
        raise RuntimeError(
            f"PX4 parameter round-trip mismatch: {written} != {read_back}"
        )
    return {
        "parameter": parameter,
        "original": original,
...
def main() -> int:
    ...
    with log_path.open("w", encoding="utf-8") as log:
        ...
        try:
            ...
            subprocess.run(["pgrep", "-f", "gz sim"], check=True, capture_output=True)
            marker.write_text(
