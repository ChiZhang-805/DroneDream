...
APPLICATION = (
    ...
    / "sim"
    / "desktop"
    / "yellow-build-attempt-20-8014750-application.v1.json"
)
ENTRY = (
    ROOT
    / "distribution"
    / "sim"
    / "desktop"
    / "invoke-yellow-build-attempt-20-8014750.ps1"
)
...
ORDINAL_NINETEEN_FAILURE_SHA256 = (
    "e3eadaec89663b5827f746272df8bf527efecd8e81bb94ab4a28eb78093c4c21"
)
...
def test_attempt_twenty_is_exact_and_not_authorized() -> None:
    ...
    assert accounting["globalCommandApplicationOrdinal"] == 20
    assert accounting["sourceApplicationPreflightOrdinal"] == 2
    assert accounting["priorLauncherBindingFailureCount"] == 1
...
def test_attempt_twenty_uses_new_absent_owned_roots() -> None:
    application = json.loads(APPLICATION.read_text(encoding="utf-8-sig"))
    roots = [
    ...
    assert history["ordinalNineteenFailureReceiptSha256"] == (
        ORDINAL_NINETEEN_FAILURE_SHA256
    )
    assert history["ordinalNineteenPermanentlyConsumed"] is True
    assert history["ordinalNineteenReuseAllowed"] is False
...
def test_launcher_binding_remains_closed_and_accepts_attempt_twenty() -> None:
    launcher = (
        ROOT / "distribution/sim/desktop/invoke-github-public-build-config.ps1"
    ).read_text(encoding="utf-8")
    assert (
        '^invoke-yellow-build-attempt-[0-9]+-[0-9a-f]{7}\\.ps1$' in launcher
    )
    assert ENTRY.name == "invoke-yellow-build-attempt-20-8014750.ps1"
    assert ".." not in ENTRY.name
    assert ENTRY.parent == ROOT / "distribution/sim/desktop"
