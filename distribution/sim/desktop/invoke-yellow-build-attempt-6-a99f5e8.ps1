...
$ProductSource = "a99f5e81893f5001ebd571d09e95b72d8afa070a"
$ProductTree = "dcc4343919d4104059a02a0327d0d4671c2b7088"
$ApplicationName = "yellow-build-attempt-6-a99f5e8-application.v1.json"
$EvidenceRoot = [IO.Path]::GetFullPath((Join-Path $PSScriptRoot "..\..\.."))
$ApplicationPath = Join-Path $PSScriptRoot $ApplicationName
$RunRoot = "C:\Users\zju20\AppData\Local\DroneDream\codex-sandboxes\software-sim\yellow-2\sim-y2-ordinal6-a99f5e8"
$SourceRoot = "C:\Users\zju20\dds6"
$BundleRoot = Join-Path $RunRoot "bundle"
$ReceiptRoot = Join-Path $RunRoot "receipt"
$CargoTargetDir = "C:\Users\zju20\AppData\Local\DroneDream\codex-cache\sim-cargo-target-a99f5e8-ordinal6"
...
function Get-PlanDocument {
    return [ordered]@{
        ...
        checkout = [ordered]@{
            strategy = "git-command-local-longpaths-worktree-add-detach-exact-commit"
...
function Invoke-ReadOnlyPreflight {
    ...
    Assert-True (-not (Test-Path -LiteralPath $CargoTargetDir)) "Owned Cargo target root already exists."
    Assert-True (-not (Test-Path -LiteralPath $BundleRoot)) "Owned output root already exists."
...
function Invoke-ExactBuild {
    ...
    Write-JsonFile ([ordered]@{
        productSourceCommit = $ProductSource
        globalAuthorizedCommandOrdinal = 6
        sourceApplicationPreflightOrdinal = 1
