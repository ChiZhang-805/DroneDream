...
$ProductSource = "573e8f991eba703bbfd6c4b35f464fbaab78903c"
$ProductTree = "57771ea1cbac8bc22453cf02f76bbb6bb547afb2"
$SemanticFingerprint = "71203209c1ce50b039c0910a6b2b3786391afd58bf2e27a1a61166f1431f39ad"
$ApplicationPath = Join-Path $PSScriptRoot "yellow-build-attempt-21-573e8f9-application.v1.json"
$EvidenceRoot = [IO.Path]::GetFullPath((Join-Path $PSScriptRoot "..\..\.."))
$SourceRoot = "C:\Users\zju20\ddss3"
$RunRoot = "C:\Users\zju20\AppData\Local\DroneDream\codex-sandboxes\software-sim\yellow-2\sim-y2-ordinal21-573e8f9"
$ReceiptRoot = Join-Path $RunRoot "receipt"
$OutputRoot = Join-Path $RunRoot "bundle"
$NpmLogsRoot = Join-Path $ReceiptRoot "npm-logs"
$CargoTargetDir = "C:\Users\zju20\AppData\Local\DroneDream\codex-cache\sim-cargo-target-573e8f9-ordinal21"
$GlobalCacheRoot = "C:\Users\zju20\AppData\Local\npm-cache"
$SnapshotOwnedBase = "C:\Users\zju20\AppData\Local\DroneDream\codex-dependencies\npm-snapshots"
$SnapshotRoot = Join-Path $SnapshotOwnedBase "npm-snapshot-ordinal21-573e8f9"
$DependencyOwnedBase = "C:\Users\zju20\AppData\Local\DroneDream\codex-dependencies\npm"
$DependencyBundleId = "npm-win32-x64-ac4e29a5f75e9dd0"
$DependencyRoot = Join-Path $DependencyOwnedBase $DependencyBundleId
$VerificationDependencyRoot = Join-Path $RunRoot "dependency-verification"
...
$SupersededBrandArtifact = "C:\Users\zju20\AppData\Local\DroneDream\codex-sandboxes\software-sim\yellow-2\sim-ui-final-ef70567-attempt1\bundle\DroneDream-Sim-1.0.0.exe"
$SupersededBrandArtifactSha256 = "9c6bf8ae11014693e6e7f723329c05f5a97b2ed359a0a28a3871868f4b65677c"
...
$CacheToolRelative = "distribution\sim\desktop\lockfile-offline-cache.mjs"
$CacheToolSha256 = "35c8104b4480915c0102bfcb0a48c82feef83c2965e55b2363ff740aed626051"
$CacheToolGitBlob = "0b2e869c1dd4ff185b2faf3550c909566ddace36"
$SeedManifestRelative = "distribution\sim\desktop\offline-cache-seeds\manifest.v1.json"
$SeedManifestSha256 = "da55441d6b81f9c1904fcad4e3e2cf3949280be10b48c7016e14105786f37156"
$SeedManifestGitBlob = "ca0418077946668d2e2c472821206b000af53310"
...
function Invoke-CacheTool {
    ...
    $tool = Join-Path $RepoRoot $CacheToolRelative
    $seedManifest = Join-Path $RepoRoot $SeedManifestRelative
    if ([IO.Path]::GetFullPath($RepoRoot).TrimEnd("\").Equals([IO.Path]::GetFullPath($EvidenceRoot).TrimEnd("\"), [StringComparison]::OrdinalIgnoreCase)) {
        Assert-True ((Get-Sha256Lower $tool) -ceq $CacheToolSha256) "Offline-cache tool drifted."
        Assert-True ((Get-Sha256Lower $seedManifest) -ceq $SeedManifestSha256) "Offline-cache seed manifest drifted."
    } else {
        Assert-DetachedGitFileBinding -RepoRoot $RepoRoot -RelativePath $CacheToolRelative -ExpectedBlob $CacheToolGitBlob
        Assert-DetachedGitFileBinding -RepoRoot $RepoRoot -RelativePath $SeedManifestRelative -ExpectedBlob $SeedManifestGitBlob
    }
    $arguments = @(
    ...
    if ($CacheMode -cne "verify-snapshot") {
        $arguments += @("--seed-manifest", $seedManifest)
    }
...
function Assert-NewRootsAbsent {
    foreach ($path in @($SourceRoot, $RunRoot, $CargoTargetDir, $SnapshotRoot, $DependencyRoot, $VerificationDependencyRoot)) {
...
function Invoke-ReadOnlyPreflight {
    ...
    Assert-True ([int]$cache.contentObjectCount -eq 324 -and [int]$cache.indexKeyCount -eq 324) "Required cache selection count drifted."
    Assert-True ([int]$cache.localSeedCount -eq 7) "Required cache seed count drifted."
    if (Test-Path -LiteralPath $FrozenArtifact -PathType Leaf) {
...
function New-DependencyManifest {
    ...
    [ordered]@{
        ...
        sourceInputs = @(
            ...
            [ordered]@{ sourcePath = "frontend/package.json"; bundlePath = "frontend/package.json"; sha256 = "ebfd1be4f47ad1089a0659ba7d994c8f650731283e4b92771c574eeb7a9f5e84" },
            [ordered]@{ sourcePath = "frontend/package-lock.json"; bundlePath = "frontend/package-lock.json"; sha256 = "926d364952dc75662cfeed48827606f44326430c065b54e9131fb499942d613b" }
        )
        toolchain = [ordered]@{
...
function Invoke-Prepare {
    ...
    Write-JsonFile ([ordered]@{ globalApplicationOrdinal = 21; preparationOrdinal = 1; buildOrdinal = 1; retry = 0 }) $AttemptLock
    ...
    New-ExclusiveAttemptDirectory -Path $VerificationDependencyRoot -CanonicalParent $RunRoot -AllowedRoot $LocalDroneDreamRoot | Out-Null
    foreach ($bundleRoot in @($DependencyRoot, $VerificationDependencyRoot)) {
        foreach ($workspace in @("desktop", "frontend")) {
            $target = Join-Path $bundleRoot $workspace
            New-ExclusiveAttemptDirectory -Path $target -CanonicalParent $bundleRoot -AllowedRoot $LocalDroneDreamRoot | Out-Null
    ...
    foreach ($bundleRoot in @($DependencyRoot, $VerificationDependencyRoot)) {
        & npm.cmd --prefix (Join-Path $bundleRoot "desktop") ci --offline --no-audit --no-fund
        Assert-True ($LASTEXITCODE -eq 0) "Desktop offline npm ci failed."
        & npm.cmd --prefix (Join-Path $bundleRoot "frontend") ci --offline --no-audit --no-fund
    ...
    $verificationInventory = Get-OfflineDependencyInventory $VerificationDependencyRoot
    foreach ($candidate in @($inventory, $verificationInventory)) {
        Assert-CleanOfflineDependencyTree -Inventory $candidate `
            -ExpectedTreeFingerprint ([string]$candidate.treeFingerprint) `
            -ExpectedEntryCount ([int]$candidate.entryCount) `
            -ExpectedFileCount ([int]$candidate.fileCount) `
            -ExpectedDirectoryCount ([int]$candidate.directoryCount) `
            -ExpectedTotalFileBytes ([UInt64]$candidate.totalFileBytes) | Out-Null
    }
    Assert-True ([string]$inventory.treeFingerprint -ceq [string]$verificationInventory.treeFingerprint) "Two clean offline dependency trees differ."
    Assert-True ([int]$inventory.entryCount -eq [int]$verificationInventory.entryCount) "Dependency entry counts differ."
    Assert-True ([int]$inventory.fileCount -eq [int]$verificationInventory.fileCount) "Dependency file counts differ."
    Assert-True ([int]$inventory.directoryCount -eq [int]$verificationInventory.directoryCount) "Dependency directory counts differ."
    Assert-True ([UInt64]$inventory.totalFileBytes -eq [UInt64]$verificationInventory.totalFileBytes) "Dependency byte counts differ."
    $core = [ordered]@{ productSource = $ProductSource; semanticFingerprint = $SemanticFingerprint; snapshotFingerprint = $snapshotSha; dependencyFingerprint = $inventory.treeFingerprint; verificationDependencyFingerprint = $verificationInventory.treeFingerprint; npmCiInvocations = 4; networkInvocations = 0; retry = 0 }
...
function Invoke-Execute {
    ...
    try {
        ...
        Write-JsonFile ([ordered]@{ state = "built-frozen-awaiting-static-acceptance"; productSource = $ProductSource; snapshotFingerprint = [string]$preparation.snapshotFingerprint; dependencyFingerprint = [string]$preparation.dependencyFingerprint; artifactPath = $FixedArtifact; artifactBytes = (Get-Item $FixedArtifact).Length; artifactSha256 = $artifactSha; buildInvocations = 1; retry = 0 }) $BuildReceipt
...
if ($Mode -ceq "Plan") {
    [ordered]@{
        ...
        semanticFingerprint = $SemanticFingerprint; requiredContentObjects = 324; requiredIndexKeys = 324; requiredLocalSeeds = 7
        ...
        verificationDependencyRootAbsent = -not (Test-Path $VerificationDependencyRoot)
