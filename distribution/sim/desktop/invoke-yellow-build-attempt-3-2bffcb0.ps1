...
$ApplicationName = "yellow-build-attempt-3-2bffcb0-application.v1.json"
$EvidenceRoot = [IO.Path]::GetFullPath((Join-Path $PSScriptRoot "..\..\.."))
$ApplicationPath = Join-Path $PSScriptRoot $ApplicationName
$RunRoot = "C:\Users\zju20\AppData\Local\DroneDream\codex-sandboxes\software-sim\yellow-2\sim-y2-ordinal3-2bffcb0"
...
function Invoke-ReadOnlyPreflight {
    ...
    Assert-True ($env:VITE_SUPABASE_URL) "Approved public VITE_SUPABASE_URL is not injected."
    Assert-True ($env:VITE_SUPABASE_PUBLISHABLE_KEY) "Approved public VITE_SUPABASE_PUBLISHABLE_KEY is not injected."
...
function Invoke-ExactBuild {
    ...
    Write-JsonFile ([ordered]@{
        productSourceCommit = $ProductSource
        globalBuildAttemptOrdinal = 3
        sourceBuildAttemptOrdinal = 1
        retryAllowed = $false
