...
$runId = "sim-red-final-fcabd99f-ordinal4"
...
try {
    ...
    if (-not $success -and $productKeyBefore -and $productKeyBefore.exists) {
        $currentProduct = Get-RegistryRecord -Path $productKey
        if ($currentProduct.exists -and
            -not $productKeyBefore.values.Contains("Installer Language") -and
            $currentProduct.values.Contains("Installer Language") -and
            [string]$currentProduct.values["Installer Language"] -in @("1033", "2052")) {
            $core = Get-SimPreferenceCoreRecord
            $expectedCore = [ordered]@{
                path = $productKeyBefore.path
                exists = $true
            ...
            if ((ConvertTo-CanonicalJson $core) -ceq (ConvertTo-CanonicalJson $expectedCore)) {
