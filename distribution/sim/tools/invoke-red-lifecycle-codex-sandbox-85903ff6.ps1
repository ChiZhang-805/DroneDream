...
$sharedRoot = Split-Path -Parent $toolRoot
. (Join-Path $toolRoot "edition-installer-lifecycle-contract.ps1")
$productSource = "573e8f991eba703bbfd6c4b35f464fbaab78903c"
$artifact = Join-Path $sharedRoot "intake\DroneDream-Sim-1.0.0.exe"
$expectedArtifactSha256 = "85903ff6a5dad93224f5396096d90f2e96e71eb5e68980df7ca2691d8001ddae"
$expectedArtifactBytes = 12070633
$staticAcceptance = Join-Path $toolRoot "yellow-build-attempt-21-573e8f9-static-accepted.v1.json"
$runId = "sim-red-final-85903ff6-ordinal1"
$runRoot = Join-Path $sharedRoot "execution"
...
function Assert-ShortcutTarget {
    ...
    $iconLocation = ([string]$record.iconLocation).Trim()
    $iconSource = if ($iconLocation -match '^(.*),\s*-?\d+$') { $Matches[1] } else { $iconLocation }
    $iconSource = $iconSource.Trim().Trim('"')
    if ([string]::IsNullOrWhiteSpace($iconSource) -or
        -not [string]::Equals(
            [IO.Path]::GetFullPath($iconSource),
            [IO.Path]::GetFullPath($ExpectedTarget),
            [StringComparison]::OrdinalIgnoreCase
        )) {
        throw "$Stage shortcut icon source is not the installed Sim executable: $Path"
    }
    return $record
}
...
try {
    ...
    $expectedProfile = [IO.Path]::GetFullPath($expectedUserProfile).TrimEnd("\")
    if ($env:USERNAME -cne $expectedUserName -or
        $currentSid -cne $expectedUserSid -or
        -not [string]::Equals($actualProfile, $expectedProfile, [StringComparison]::OrdinalIgnoreCase)) {
        throw "Lifecycle execution requires the exact disposable CodexSandboxOffline identity and profile."
    }
    foreach ($profileScopedPath in @($env:LOCALAPPDATA, $env:APPDATA, $env:TEMP, $env:TMP)) {
        if ([string]::IsNullOrWhiteSpace($profileScopedPath)) {
            throw "A required disposable-user profile path is empty."
        }
    }
    if (-not ([IO.Path]::GetFullPath($env:LOCALAPPDATA)).StartsWith($expectedProfile + "\", [StringComparison]::OrdinalIgnoreCase) -or
        -not ([IO.Path]::GetFullPath($env:APPDATA)).StartsWith($expectedProfile + "\", [StringComparison]::OrdinalIgnoreCase)) {
        throw "Lifecycle profile-scoped paths escaped the disposable user profile."
    }
    ...
    $contractPath = Join-Path $toolRoot "edition-installer-lifecycle-contract.ps1"
    $applicationDocument = Get-Content -LiteralPath $applicationFull -Raw | ConvertFrom-Json
    if (-not $applicationFull.StartsWith($toolRoot + [IO.Path]::DirectorySeparatorChar, [StringComparison]::OrdinalIgnoreCase) -or
        ...
        (Get-Item -LiteralPath $applicationFull).Length -ne $ExpectedApplicationBytes -or
        [string]$applicationDocument.editionId -cne "sim" -or
        [string]$applicationDocument.sourceSeparation.productSourceCommit -cne $productSource -or
        [string]$applicationDocument.artifact.sha256 -cne $expectedArtifactSha256 -or
    ...
    $receipt.preflight = [ordered]@{
        toolEvidenceHead = $ExpectedToolHead
        stagedToolBundleIdentity = "pass"
        disposableUserName = $env:USERNAME
        disposableUserSid = $currentSid
        disposableUserProfile = $actualProfile
        artifactRehash = "pass"
