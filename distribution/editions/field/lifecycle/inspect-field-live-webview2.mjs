...
if (!endpoint || !phase || !outputPath) {
  throw new Error("Usage: inspect-field-live-webview2.mjs <endpoint> <phase> <output.json>");
}
...
try {
  ...
  await page.waitForSelector('.app-shell[data-brand-edition="field"]', {
    state: "visible",
    timeout: 30_000,
  });
  ...
  if (initialLocale !== "en" && initialLocale !== "zh-CN") {
    throw new Error(`Unexpected initial Field locale: ${initialLocale}`);
  }
  ...
  if ((await languageButtons.count()) !== 2) {
    throw new Error("The Field settings dialog must expose exactly two language buttons.");
  }
  ...
  if (!initialDialogTitle || !finalDialogTitle || initialDialogTitle === finalDialogTitle) {
    throw new Error("The live Field settings title did not change across EN/ZH.");
  }

  const brand = await page.locator('img.brand-lockup[data-brand-edition="field"]').first().evaluate(
    (image) => ({
  ...
  if (!brand.complete || brand.naturalWidth <= 0 || brand.naturalHeight <= 0) {
    throw new Error("The canonical Field lockup did not render in WebView2.");
  }
  if (brand.naturalWidth !== 2581 || brand.naturalHeight !== 218) {
    throw new Error(`Unexpected canonical Field lockup dimensions: ${brand.naturalWidth}x${brand.naturalHeight}`);
  }

  const safety = await page.evaluate(() => {
    const validatedCount = document.querySelector(".field-sidebar-status strong")?.textContent?.trim();
    const criticalBanner = document.querySelector('.field-critical-banner[role="alert"]');
    const controlButtons = [...document.querySelectorAll(".field-control-buttons button")];
    const deniedActions = [...document.querySelectorAll(".field-action-matrix > div")];
    return {
      validatedCount,
      criticalBannerVisible: Boolean(criticalBanner),
      controlButtonCount: controlButtons.length,
      disabledControlButtonCount: controlButtons.filter((button) => button.disabled).length,
      deniedActionCount: deniedActions.length,
      deniedActionLabels: deniedActions.map((item) => item.textContent?.trim() ?? ""),
    };
  });
  if (
    safety.validatedCount !== "0" ||
    !safety.criticalBannerVisible ||
    safety.controlButtonCount !== 2 ||
    safety.disabledControlButtonCount !== 2 ||
    safety.deniedActionCount !== 3
  ) {
    throw new Error("The live Field zero-pack hardware-deny surface did not match the frozen contract.");
  }
  ...
  const screenshotPath = resolve(dirname(outputPath), `${phase}-field-webview2.png`);
