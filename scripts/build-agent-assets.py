...
def verified_image(
    path: Path, expected_sha256: str, expected_size: tuple[int, int]
) -> Image.Image:
...
def build_mark() -> Image.Image:
    standard_mark = verified_image(
        STANDARD_MARK_PATH, STANDARD_MARK_SHA256, (1024, 1024)
    )
    return recolor_mark(standard_mark, PALETTE)


def build_lockup() -> Image.Image:
    standard = verified_image(
        STANDARD_LOCKUP_PATH, STANDARD_LOCKUP_SHA256, (2581, 218)
    )
    prefix = recolor_mark(
    ...
    separator = recolor_mark(
        standard.crop(
            (SEPARATOR_START_X, 0, SEPARATOR_END_X + 1, CANVAS_HEIGHT)
        ),
        PALETTE,
    )
...
def main() -> None:
    ...
    if args.check:
        ...
        for path, payload in outputs.items():
            print(
                f"Verified {path.relative_to(REPO).as_posix()} ({sha256(payload)})"
            )
        return
    for path, payload in outputs.items():
