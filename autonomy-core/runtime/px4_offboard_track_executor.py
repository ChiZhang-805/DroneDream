...
class MavsdkOffboardClient:
    ...
    async def execute_camera_command(self, parameters: dict[str, Any]) -> dict[str, Any]:
        ...
        if command == "take_photo":
            await camera.take_photo(component_id)
        elif command == "start_video":
            await camera.start_video(component_id)
        elif command == "stop_video":
            await camera.stop_video(component_id)
        else:
    ...
    async def execute_payload_command(self, parameters: dict[str, Any]) -> dict[str, Any]:
        ...
        if protocol == "mavsdk-actuator":
            actuator_index = int(parameters["actuator_index"])
            actuator_value = float(parameters["actuator_value"])
        ...
        output_topic = str(parameters["output_topic"])
        ...
        try:
            ...
            publisher_stdout, publisher_stderr = await asyncio.wait_for(
                publisher.communicate(), timeout=3.0
            )
            if publisher.returncode != 0:
                details = (publisher_stderr or publisher_stdout).decode(errors="replace").strip()
                raise RuntimeError(f"Gazebo payload command failed: {details}")
            state_stdout, state_stderr = await asyncio.wait_for(listener.communicate(), timeout=3.0)
        finally:
            if listener.returncode is None:
                listener.kill()
                await listener.wait()
        if listener.returncode != 0:
            details = (state_stderr or state_stdout).decode(errors="replace").strip()
            raise RuntimeError(f"Gazebo payload state readback failed: {details}")
        state = state_stdout.decode(errors="replace").strip()
        match = re.search(r"data:\s*(true|false)", state, flags=re.IGNORECASE)
        expected_detached = operation == "detach"
        if match is None or (match.group(1).lower() == "true") != expected_detached:
            raise RuntimeError(
    ...
    async def sample_gazebo_pose(self, parameters: dict[str, Any]) -> dict[str, float | str]:
        ...
        if gz is None:
            raise RuntimeError("Gazebo transport executable 'gz' is unavailable")
        topic = str(parameters["target_pose_topic"])
        ...
        if process.returncode != 0:
            ...
            raise RuntimeError(f"Gazebo target pose read failed: {details}")
        payload = stdout.decode(errors="replace")
        target_name = str(parameters.get("target_model_name", "")).strip()
        numeric_position = (
            r"position\s*\{[^}]*x:\s*([-+0-9.eE]+)[^}]*"
            r"y:\s*([-+0-9.eE]+)[^}]*z:\s*([-+0-9.eE]+)"
        )
        if target_name:
            position = re.search(
                rf'name:\s*"{re.escape(target_name)}".*?{numeric_position}',
                payload,
                flags=re.DOTALL,
            )
            if position is None:
                raise RuntimeError(f"Gazebo target pose did not contain model: {target_name}")
        else:
            position = re.search(numeric_position, payload, flags=re.DOTALL)
        if position is None:
            raise RuntimeError("Gazebo target pose response has no finite position")
        x, y, z = (float(position.group(index)) for index in range(1, 4))
        if not all(math.isfinite(value) for value in (x, y, z)):
            raise RuntimeError("Gazebo target pose response has non-finite coordinates")
        return {"x": x, "y": y, "z": z, "topic": topic, "target_name": target_name}
...
class FakeOffboardClient:
    ...
    async def execute_camera_command(self, parameters: dict[str, Any]) -> dict[str, Any]:
        return {"confirmed": True, "transport": "fake-camera", **parameters}

    async def execute_payload_command(self, parameters: dict[str, Any]) -> dict[str, Any]:
        return {"confirmed": True, "transport": "fake-payload", **parameters}
