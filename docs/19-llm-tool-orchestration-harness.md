...
| objective/estimand/selection audit | Outcome Contract compiler 1.3 and Selection Key 1.0 now remove the hidden failure threshold, hard-penalty shortcut, multi-bound observation overwrite, simultaneous objective-vector/scalar-loss acquisition, flat seed estimator, surviving-row case-weight drift, and rounded acceptance decisions; current schemas still admit free metric names, raw custom-metric dependencies are unregistered, observed min/max ranges and random scalarizations move preferences, and compatibility reports are not yet immutable outcome-evidence projections | retain the immutable contract, hierarchical estimator, versioned promotion projection, one-representation rule, and shared lexicographic key; add a registered metric DAG, explicit non-success/censoring/risk algebra, fixed transforms/scalarization/Pareto references, immutable `CandidateOutcomeEvidenceV1`, and one shared projection for optimizer rank, reward, promotion, winner freeze, report, and replay |
