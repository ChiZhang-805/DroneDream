...
| objective/estimand/selection audit | Outcome Contract compiler 1.2 and Selection Key 1.0 now remove the hidden failure threshold, hard-penalty shortcut, multi-bound observation overwrite, simultaneous objective-vector/scalar-loss acquisition, flat seed estimator, and surviving-row case-weight drift; current schemas still admit free metric names, raw custom-metric dependencies are unregistered, observed min/max ranges and random scalarizations move preferences, and acceptance recomputes rounded metrics differently from tool ranking | retain the immutable contract, hierarchical estimator, one-representation rule, and shared lexicographic key; add a registered metric DAG, explicit non-success/censoring/risk algebra, fixed transforms/scalarization/Pareto references, immutable `CandidateOutcomeEvidenceV1`, and one shared projection for optimizer rank, reward, acceptance, promotion, winner freeze, report, and replay |
