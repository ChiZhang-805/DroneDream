...
| `backend/app/orchestration/aggregation.py` and `acceptance.py` | generation completion, case-weighted reliability denominators, hierarchical usable-replicate means before fixed case weights, seed-level worst constraints, training-field preference, and continuation boundary | replace exclusive GPT/CMA/experimental continuation branches with one dispatcher; emit immutable `CandidateOutcomeEvidenceV1`; finish one shared projection for optimizer ranking, acceptance, validation promotion, winner freeze, and reports; remove pre-ranking rounding and alternate acceptance arithmetic |
...
| objective/estimand/selection audit | Outcome Contract compiler 1.5 and Selection Key 1.0 now remove the hidden failure threshold, hard-penalty shortcut, multi-bound observation overwrite, simultaneous objective-vector/scalar-loss acquisition, flat seed estimator, surviving-row case-weight drift, rounded acceptance decisions, implicit adapter raw metrics, and known reliability/composite overlap; the reviewed registry still lacks a complete component DAG for every derived metric, observed min/max ranges and random scalarizations move preferences, and compatibility reports are not yet immutable outcome-evidence projections | retain the immutable registered-metric contract, known dependency guards, hierarchical estimator, versioned promotion projection, one-representation rule, and shared lexicographic key; complete the metric DAG, explicit non-success/censoring/risk algebra, fixed transforms/scalarization/Pareto references, immutable `CandidateOutcomeEvidenceV1`, and one shared projection for optimizer rank, reward, promotion, winner freeze, report, and replay |
